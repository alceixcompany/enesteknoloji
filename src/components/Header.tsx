  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 border-b border-black/10 bg-[#f7f2e8]/95 backdrop-blur-xl transition-all ${scrolled ? 'shadow-sm' : ''}`}>
      <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
        ...
        <a href="tel:+905365278175" className="hidden items-center gap-2 rounded-full bg-[var(--ink)] px-5 py-3 text-xs font-bold text-white transition hover:-translate-y-0.5 hover:bg-black sm:flex"><FiPhone /> Bizi Ara</a>
      ...
      {open && (
        <nav className="border-t border-black/10 bg-[#f7f2e8] px-5 py-5 shadow-xl lg:hidden">
          ...
          <a href="tel:+905365278175" className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-[var(--taxi-yellow)] px-5 py-4 font-bold text-[var(--ink)]"><FiPhone /> Bizi Ara</a>
        </nav>
      )}
    </header>
  );
